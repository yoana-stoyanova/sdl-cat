            ...
            for (int w = 0; w < rx * 2; w++) {
                for (int h = 0; h < ry * 2; h++) {
                    ...
                    if ((dx*dx)*ry*ry + (dy*dy)*rx*rx <= rx*rx*ry*ry) {
                        SDL_RenderDrawPoint(SDLRen, x0 + dx, y0 + dy);
                    }
                }
            }
        }
    
        void drawSin(int x1, int x2, int y0, int height) {
            double amplitude = height;

            for (int x = x1; x <= x2; x++) {
                double t = 3.14 * (x - x1) / (x2 - x1);
                int y = static_cast<int>(y0 - amplitude * sin(t));

                SDL_RenderDrawLine(SDLRen, x, y, x, y0);
            }
        }
    }